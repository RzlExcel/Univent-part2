...
<div class="min-h-screen bg-slate-50/50 pt-28 pb-20 px-4 relative overflow-hidden">
    ...
    <div class="max-w-6xl mx-auto relative">
        {{-- Back Button --}}
        <a href="{{ route('admin.event-list') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 text-slate-600 font-bold rounded-2xl hover:bg-slate-50 transition-all mb-10 shadow-sm text-sm">
                ...
                <div class="sticky top-28 space-y-6">
                    ...
                    <div class="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-xl shadow-slate-200/50 space-y-8">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            {{-- Tambahkan Registration Link ini tepat di bawah div Contact Person --}}
                        <div class="flex items-start gap-4 mt-6 pt-6 border-t border-slate-100">
                            ...
                            <div>
                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Registration Link</p>
                                <a href="{{ $event->registration_link }}" target="_blank" class="text-sm font-bold text-blue-600 hover:underline break-all">
                    ...
                    {{-- Hapus tombol Register warna pink, ganti dengan ini --}}
                    <div class="mt-6 flex flex-col gap-3">
                        @if($event->status !== 'approved')
                            <form action="{{ route('admin.events.approve', $event->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full bg-green-500 hover:bg-green-600 text-white font-bold py-4 rounded-[2rem] transition-all shadow-lg shadow-green-200">
                                    APPROVE EVENT
                                </button>
                            </form>
                        @endif
                        @if($event->status !== 'rejected')
                            <form action="{{ route('admin.events.reject', $event->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full bg-red-50 hover:bg-red-100 text-red-500 font-bold py-4 rounded-[2rem] transition-all">
                                    REJECT EVENT
                                </button>
                            </form> 
                        @endif
