    ...
    <table class="w-full text-left border-collapse">
        <thead>
            <tr class="bg-slate-50/50 border-b border-slate-100">
                ...
                <th class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-center">Status
                </th>
                <th class="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-right">Aksi
                </th>
            </tr>
        </thead>
                ...
                <tr class="hover:bg-slate-50/30 transition-colors group">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-sm font-black text-slate-900 line-clamp-1 italic tracking-tight">
                                    {{ $event->event_title }}</p>
                                <p class="text-[10px] font-bold text-red-500 uppercase tracking-tighter">by
                    ...
                    <td class="px-6 py-4 text-center">
                        <span
                            class="px-3 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-full 
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            @if ($showActions || $showRevert)
                                ...
                                @if (($showActions && $event->status === 'pending') || ($showRevert && $event->status === 'rejected'))
                                    <form action="{{ route('admin.events.approve', $event->id) }}" method="POST"
                                        ...
                                        @csrf
                                        <button type="submit"
                                            class="p-2.5 bg-green-100 text-green-600 rounded-xl hover:bg-green-600 hover:text-white transition-all shadow-sm"
                                            title="Accept">
                                ...
                                @if (($showActions && $event->status === 'pending') || ($showRevert && $event->status === 'approved'))
                                    <form action="{{ route('admin.events.reject', $event->id) }}" method="POST"
                                        ...
                                        @csrf
                                        <button type="submit"
                                            class="p-2.5 bg-red-100 text-red-600 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm"
                                            title="Reject">
                            ...
                            <form action="{{ route('admin.events.delete', $event->id) }}" method="POST"
                                ...
                                @method('DELETE')
                                <button type="button"
                                    class="p-2.5 bg-white border border-slate-200 text-slate-400 rounded-xl hover:bg-red-50 hover:text-red-500 hover:border-red-200 transition-all shadow-sm btn-delete-sw"
                                    title="Delete">
