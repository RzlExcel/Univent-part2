...
<body class="bg-slate-50 text-slate-900 antialiased">
    ...
    <nav x-data="{ mobileMenu: false, profileMenu: false }"
                        ...
                        <div class="relative">
                            <button @click="profileMenu = !profileMenu"
                                ...
                                <div class="h-9 w-9 rounded-full ring-2 ring-red-500/20 overflow-hidden bg-slate-200">
                                    <img src="{{ Auth::user()->avatar ? (strlen(Auth::user()->avatar) > 200 ? 'data:image/jpeg;base64,' . Auth::user()->avatar : asset('storage/' . Auth::user()->avatar)) : asset('images/default-avatar.svg') }}"
                                ...
                                <div class="max-h-[350px] overflow-y-auto">
                                    @if (auth()->user()->unreadNotifications->count() > 0)
                                        @foreach (auth()->user()->unreadNotifications as $notification)
                                            ...
                                            @if ($isRejected)
                                                <div
                                                    class="block p-4 border-b border-slate-50 bg-white opacity-80 transition cursor-default">
                                                @else
                                                    <a href="{{ route('notifications.click', $notification->id) }}"
                                                        class="block p-4 border-b border-slate-50 hover:bg-slate-50 transition cursor-pointer">
                                            @endif
                                            ...
                                            <div class="flex gap-3">
                                                ...
                                                <div>
                                                    <p class="text-sm text-slate-700 leading-snug">
                                                        {!! $notification->data['message'] ?? 'Ada pembaruan.' !!}</p>
                                                    <span
                                            ...
                                            <div class="flex gap-3">
                                                ...
                                                <div>
                                                    <p class="text-sm text-slate-700 leading-snug">
                                                        {!! $notification->data['message'] ?? 'Ada pembaruan.' !!}</p>
                                                    <span
                                                        class="text-[10px] font-bold text-slate-400 mt-1 block">{{ $notification->created_at->diffForHumans() }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
        ...
        <div x-show="mobileMenu" x-cloak
            ...
            @auth
                @if (auth()->user()->hasRole('eo') || auth()->user()->hasRole('admin'))
                    <a href="{{ route('submit-event.form') }}" class="block text-base font-bold text-slate-700">Submit
                        Event</a>
                @endif
            @endauth
            ...
            @auth
                ...
                <a href="{{ route('user.event.history') }}" class="block text-base font-bold text-slate-700">Event
                    History</a>
        ...
        <div class="max-w-7xl mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-12">
            ...
            <div>
                ...
                <ul class="space-y-3 text-slate-400 text-sm">
                    ...
                    @auth
                        @if (auth()->user()->hasRole('eo') || auth()->user()->hasRole('admin'))
                            <li><a href="{{ route('submit-event.form') }}" class="hover:text-red-400 transition">Submit
                                    Event</a></li>
                        @endif
                    @endauth
                </ul>
                ...
                <div @click.away="openEoModal = false" x-show="openEoModal"
                    ...
                    <form action="{{ route('user.request-eo') }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="eo_pic_name" required value="{{ Auth::user()->name }}"
                                class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-sm font-medium text-slate-700 outline-none">
                        </div>
    ...
    <div class="fixed bottom-6 right-6 z-50" x-data="{ hover: false }">
        ...
        <a href="{{ route('contact') }}" @mouseenter="hover = true" @mouseleave="hover = false"
