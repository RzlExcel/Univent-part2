...
class ApiEventService {
  ...
  Future<Map<String, dynamic>> generateAiDescription(
    String title,
    String category,
  ) async {
    final url = Uri.parse("$baseUrl/event/generate-description");
    try {
      final response = await http.post(
        ...
        body: jsonEncode({"title": title, "category_name": category}),
      );
      if (response.statusCode == 200) {
        return jsonDecode(response.body);
      }
      ...
      return {
        "success": false,
        "message": errorData['message'] ?? "Gagal memuat AI.",
      };
    } catch (e) {
      return {"success": false, "message": "Eror Koneksi AI: $e"};
    }
  }
}
