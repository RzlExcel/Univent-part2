...
class _NotificationPageState extends State<NotificationPage> {
  ...
  Future<void> _deleteAllNotifications() async {
    final response = await ApiNotificationService.clearAllNotifications();

    if (response['success'] == true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Semua notifikasi berhasil disapu bersih, bos!"),
        ),
      );
      _refresh(); // Segarkan layar
    } else {
      // 🔴 UBAH BAGIAN INI: Ambil pesan error asli dari Dio/Laravel 🔴
      final errorResmi = response['message'] ?? 'Gagal tanpa alasan.';

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text("GAGAL BOS! Detailnya: $errorResmi"),
          backgroundColor: Colors.red[800],
          duration: const Duration(
            seconds: 5,
          ), // Biar nampang agak lamaan buat dibaca
        ),
      );
    }
  }
            ...
            onPressed: () {
              ...
              showDialog(
                ...
                builder: (context) => AlertDialog(
                  ...
                  content: const Text(
                    "Apakah bos yakin ingin menghapus seluruh riwayat notifikasi ini?",
                  ),
                  actions: [
                    TextButton(
                      onPressed: () => Navigator.pop(context),
                    ...
                    TextButton(
                      onPressed: () {
                        ...
                        _deleteAllNotifications(); // Eksekusi fungsi hapus di atas tadi
                      },
                      child: const Text(
                        "Ya, Hapus",
                        style: TextStyle(
