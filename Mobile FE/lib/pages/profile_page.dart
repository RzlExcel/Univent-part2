...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void initState() {
    ...
    _profileFuture = Future.delayed(const Duration(milliseconds: 500), () {
      return ApiAuthService.getUserProfile();
    });
  }
    ...
    return SafeArea(
      child: FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          // 👇 PASANG 2 CCTV INI DI SINI 👇
          print("🚨 INVESTIGASI PROFIL - Data dari API: ${snapshot.data}");
          if (snapshot.hasError) {
            print(
              "🚨 INVESTIGASI PROFIL - Ada Error Sistem: ${snapshot.error}",
            );
          }
                            ...
                            Expanded(
                              ...
                              child: OutlinedButton.icon(
                                onPressed: () async {
                                  ...
                                  // 3. HAPUS SEMUA INGATAN (isLoggedIn, token, dll)
                                  await prefs.clear();

                                  // 4. Hapus token di memori aplikasi
                                  ApiAuthService.token = null;

                                  // 5. Tendang user kembali ke halaman Login (Bukan ke UniventHomePage)
                                  if (!context.mounted) return;
                                  Navigator.pushAndRemoveUntil(
                                    context,
                                    MaterialPageRoute(
                                      // ARAHKAN KE LOGIN PAGE KARENA SUDAH LOGOUT
                                      builder: (context) => const LoginPage(),
                                    ),
                                    (route) => false,
                                  );
